...
namespace Лаба3
{
    ...
    public class HealthStats : IHealthStats // Класс, реализующий статистику здоровья персонажа.
    {
        ...
        {
            Health += amount;
            Console.WriteLine($"Здоровье увеличилось на {amount}! Текущее здоровье игрока: {Health}");
        }
        public void RemoveHealth(int amount) // Уменшение здоровья персонажа на заданную величину.
        {
            Health -= amount;
            Console.WriteLine($"Здоровье уменьшилось на {amount}!Текущее здоровье игрока: {Health}");
        }
    ...
    public class Inventory : IInventory
    {
        ...
        {
            Objects.Add(obj);
            Console.WriteLine($"{obj} был добавлен в инвентарь!");
        }
        ...
        {
            Objects.Remove(obj);
            Console.WriteLine($"{obj} был изъят из инвентаря!");
        }
        ...
        {
            Keys += keys;
            Console.WriteLine("Ключ был добавлен в инвентарь!");
            Console.WriteLine($"Текущее количество ключей: {Keys}");
        }
    }
